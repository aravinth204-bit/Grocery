...
const StatCard = ({ title, rawValue, prefix = '', suffix = '', icon, trend, isPositive, delay }) => {
  const counted = useCountUp(rawValue);
  const display = prefix + (Number.isInteger(rawValue) ? Math.round(counted) : counted.toFixed(2)) + suffix;
  ...
  >
    ...
    <h3 className="text-4xl font-black text-slate-900 dark:text-white relative z-10 tracking-tight tabular-nums">{display}</h3>
  </motion.div>
  );
};
  ...
  return (
    <div className="flex flex-col gap-8 pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Total Sales" rawValue={stats.totalRevenue || 0} prefix="$" icon={<DollarSign size={24} />} trend="+12.5%" isPositive={true} delay={0.1} />
        <StatCard title="Total Orders" rawValue={stats.totalOrders || 0} icon={<ShoppingCart size={24} />} trend="+8.2%" isPositive={true} delay={0.2} />
        <StatCard title="Total Products" rawValue={stats.totalProducts || 0} icon={<Package size={24} />} trend="-2.4%" isPositive={false} delay={0.3} />
        <StatCard title="Total Customers" rawValue={stats.totalUsers || 0} icon={<Users size={24} />} trend="+15.3%" isPositive={true} delay={0.4} />
      </div>
